...
class ModIntervalTree(MutableSet):
    """
    ...
    Initialize a tree from an iterable set of Intervals in O(n * log n)::
        >>> tree = ModIntervalTree([Interval(-10, 10), Interval(-20.0, -10.0)])
        >>> tree
    ...
    def __init__(self, intervals=None):
        ...
        for iv in intervals:
            if iv.is_null():
                raise ValueError(
                    "ModIntervalTree: Null Interval objects not allowed in IntervalTree:"
                    " {0}".format(iv)
                )
    ...
    def copy(self):
        ...
        :rtype: IntervalTree
        """
        return ModIntervalTree(iv.copy() for iv in self)
    ...
    def add(self, interval):
        ...
        if interval.is_null():
            raise ValueError(
                "ModIntervalTree: Null Interval objects not allowed in ModIntervalTree:"
                " {0}".format(interval)
            )
    ...
    def union(self, other):
        ...
        and other.
        """
        return ModIntervalTree(set(self).union(other))
    ...
    def split_overlaps(self, data_reducer=None):
        if not self:
            return
        ...
        for lbound, ubound in zip(bounds[:-1], bounds[1:]):
            if data_reducer and len(self[lbound]) > 1:
                iv_data = reduce(data_reducer, [iv.data for iv in self[lbound]])
                new_ivs.add(Interval(lbound, ubound, iv_data))
            else:
    ...
    def merge_overlaps(self, data_reducer=None, data_initializer=None, strict=True, data_compare=None):
        """
        ...
        comparator = data_compare if data_compare is not None else lambda x, y: not strict

        for higher in sorted_intervals:
            if merged:  # series already begun
                lower = merged[-1]
                if (higher.begin < lower.end or
                        comparator(current_reduced[0], higher.data) and higher.begin == lower.end):  # should merge
    ...
    def __eq__(self, other):
        ...
        """
        return (
            isinstance(other, ModIntervalTree) and
    ...
    def __repr__(self):
        ...
        ivs = sorted(self)
        if not ivs:
            return "ModIntervalTree()"
        else:
            return "ModIntervalTree({0})".format(ivs)
    ...
    def __reduce__(self):
        ...
        :rtype: tuple
        """
        return ModIntervalTree, (sorted(self.all_intervals),)
