...
class MultiResourceFixedPointSegmentBlockPreemptionBudgetScheduler(Scheduler):
    def preprocess_taskset(self, taskset):
        return taskset
...
class MultipleResourceSegmentBlockPreemptionBudgetScheduler(Scheduler):
    def schedule_tasks(self, dagset, topology):
        # Convert DAGs into tasks
        tasks = {}
        resources = set()
        ...
        # For each set of tasks use NPEDFScheduler
        scheduler = MultiResourceFixedPointSegmentBlockPreemptionBudgetScheduler()
        schedules = []
        for taskset in tasksets:
            schedule, valid = scheduler.schedule_tasks(taskset, topology)
            schedules.append((taskset, schedule, valid))
