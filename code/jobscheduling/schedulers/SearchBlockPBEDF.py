...
class MultiResourceInconsiderateFixedPointSegmentPreemptionBudgetScheduler(MultiResourceInconsiderateFixedPointBlockPreemptionBudgetScheduler):
    def schedule_tasks(self, taskset, topology):
        original_taskset = taskset
        hyperperiod = get_lcm_for([t.p for t in original_taskset])
        logger.debug("Computed hyperperiod {}".format(hyperperiod))
        ...
        while taskset:
            ...
            for segment_interval, segment_task in preemption_point_intervals:
                ...
                for resource, itree in segment_intervals:
                    offset_itree = IntervalTree(
                        [Interval(i.begin + segment_start - segment_task.a, i.end + segment_start - segment_task.a) for i in itree])
            ...
            if taskset:
                ...
                earliest = min_chop
                for resource in next_task.resources:
                    if any([global_resource_occupations[resource].overlap(begin, end) for begin, end, _ in resource_intervals[resource]]):
                        import pdb
                        pdb.set_trace()
                for resource in next_task.resources:
...
class MultiResourceConsiderateFixedPointSegmentBlockPreemptionBudgetScheduler(MultiResourceInconsiderateFixedPointSegmentBlockPreemptionBudgetScheduler):
    def schedule_tasks(self, taskset, topology):
        original_taskset = taskset
        hyperperiod = get_lcm_for([t.p for t in original_taskset])
        logger.debug("Computed hyperperiod {}".format(hyperperiod))
...
class MultiResourceConsiderateFixedPointSegmentPreemptionBudgetScheduler(MultiResourceInconsiderateFixedPointSegmentBlockPreemptionBudgetScheduler):
    def schedule_tasks(self, taskset, topology):
        original_taskset = taskset
        hyperperiod = get_lcm_for([t.p for t in original_taskset])
        logger.debug("Computed hyperperiod {}".format(hyperperiod))
